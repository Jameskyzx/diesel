...
上述只是当前本地工作树状态，不表示公网数据已清理或新代码已部署。关闭运营层事故仍需
部署后读回 `/api/products`、对目标 PostgreSQL 核验 `products_power_check` 与
`power_max_kw > power_min_kw`，并归档/纠正错误产品—来源关联。
