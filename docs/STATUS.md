...
- 本轮质量门通过：`pnpm lint`、`pnpm typecheck`、856 条 Vitest、`pnpm build`、
  默认 Playwright 60 passed / 6 skipped；另在作品集 Demo 模式运行专属点名产品用例，
  桌面与移动端 2 passed。浏览器走查确认“演示匹配”边界和详情到预填对话链路。
  这些是工程回归结果，不是用户效果 KPI。
