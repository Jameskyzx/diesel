...
- 提示注入与事实边界：所有提取文字都包在显式 `BEGIN/END USER-UPLOADED ATTACHMENT`
  ...
  附件提取、描述、转录或翻译的请求，才允许在固定“附件尚未核验”提示后回答；即使模型
  在 `auto` 模式主动调用工具并取得充分证据，该前缀仍不可移除。
...
- 验证方式：Zod/路由测试覆盖远程 URL、结构/截断、历史附件、数量和大小边界；图片测试
  ...
  transform 测试同时锁定纯附件概述可放行、`auto` 模式充分工具结果后仍保留未核验前缀、
  混合事实问题失败关闭；桌面/移动 Playwright 覆盖选择、预览、移除与错误状态。生产构建
